jQuery(document).ready(function() {
  ...
  content = false;
  video = false;
  if( jQuery('body').attr('video')=='1' ){ video = true; }
  ...
  jQuery(window).scroll(function(){
    ...
    logo.velocity({ translateY: -delta*0.7 },0);
    title.velocity({ translateY: -delta*0.7 },0);
    sky.velocity({ translateY: delta*0.45 },0);
    if( delta > 150 && !content ){
      content = true;
      coatOpened.css({ opacity: 1 });
      coatClosed.css({ opacity: 0 });
    } else if( delta <= 150 && content ){
      content = false;
      coatOpened.css({ opacity: 0 });
      coatClosed.css({ opacity: 1 });
    }
  });
  ...
  function cantClose(){
    gbmouse.click(function(e){
      ...
      setTimeout(function(){ gboverlay.removeClass('noclick'); }, 150);
    });
  }
  ...
  function gbClose(enable){
    if( enable ){
      ...
      gbmouse.click(function(e){
        ...
        setTimeout(function(){ jQuery('#inner').removeClass('hidden'); }, ts/4);
        setTimeout(function(){
          gboverlay.css({ display: 'none' });
      ...
      gbmouse.mousemove(function(e){
        close.addClass('show');
        close.css({ top: e.pageY - gbmouse.offset().top, left: e.pageX });
      });
  ...
  function generateLevel2(){
    ...
    progress.velocity({ width: sbLength }, duration, 'linear', function(){
      jQuery('#sbcirca3, #step3').addClass('active');
      if( !video ){
        changeText(level3.eq(1).html());
        gblogo.addClass('hide');
        setTimeout(function(){ wholder.addClass('show'); }, 600);
      ...
      setTimeout(function(){ code.find('span.final').addClass('active'); },10);
    });
  }
...
jQuery(window).load(function(){
  ...
  if( jQuery('#rdbox').is('*') ){
    ...
    jQuery('#rdclose, #rbdismiss').click(function(){
      ...
      setTimeout(function(){ jQuery('#rdoverlay').remove(); }, 600);
    });
  } else {
    jQuery('#inner').removeClass('preload');
  }
